...
/*
TODO:
	Create a method that draws paths in a different color than the PFS
...
void Graphics::HandleEventOnTimer(void)
{
	...
	{
	    if(m_method == 1)      m_planner->ExtendERRT();
...
//MAKE EDITS UNDER HERE
/*
	if global variable notPFS = 1 (meaning we want to draw the library path)
		use the code below to draw a path except use a different color.
			- make sure that the path is drawn and does not flag that its reach the goal yet	
*/
	...
	for(int i = 0; i < n; ++i)
	    glVertex2dv(m_planner->m_vertices[i]->m_state); //chosing the vertices to draw
	glEnd();//stops drawing
...
/*
	This might suffice as is however, issues that might arise:
		how to call this within the MP.cpp 



void Graphics::drawPath(std::vector<int> *path){
	glColor3f(1,0,0); //r,g,b - 100% red
	glBegin(GL_POINTS);	// starts drawing POINTS
	const int n = path->size();
	for(int i = 0; i < n; ++i)
	    glVertex2dv(path[i]); //chosing the vertices to draw
	glEnd();//stops drawing
}
*/
